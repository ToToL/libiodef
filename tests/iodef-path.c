...
static void set_value_check(iodef_document_t *iodef, const char *paths,
                            const char *str_value, libiodef_bool_t verify_get)
{
        iodef_path_t *path;
        iodef_value_t *value;
        libiodef_string_t *str;
        libiodef_string_t *res;

        assert(iodef_path_new_fast(&path, paths) == 0);
        assert(libiodef_string_new_ref(&str, str_value) == 0);
        ...
        assert(libiodef_string_new(&res) == 0);
        assert(iodef_value_to_string(value, res) >= 0);
        assert(strcmp(str_value, libiodef_string_get_string(res)) == 0);
        libiodef_string_destroy(res);
...
int main(void)
{
        ...
        struct {
                const char *path;
                int depth;
                libiodef_bool_t has_list;
                libiodef_bool_t ambiguous;
                libiodef_bool_t successful;
        } plist[] = {
