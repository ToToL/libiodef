...
/*
 * This code include an heavily modified version of the libiodef-strbuf
 * API made by Krzysztof Zaraska, that is now part of libiodef-string.
 */
...
#define LIBIODEF_ERROR_SOURCE_DEFAULT LIBIODEF_ERROR_SOURCE_STRING
 ...
 */
#define LIBIODEF_STRING_OWN_STRUCTURE  0x1

/*
 * String data may be free'd
 */
#define LIBIODEF_STRING_OWN_DATA       0x2




#if ! defined (LIBIODEF_VA_COPY)

# if defined (__GNUC__) && defined (__PPC__) && (defined (_CALL_SYSV) || defined (_WIN32))
#  define LIBIODEF_VA_COPY(ap1, ap2)     (*(ap1) = *(ap2))

# elif defined (LIBIODEF_VA_COPY_AS_ARRAY)
#  define LIBIODEF_VA_COPY(ap1, ap2)     memmove ((ap1), (ap2), sizeof(va_list))
...
#define STRING_RETURN_IF_INVALID(str, len) do {                                                                \
        libiodef_return_val_if_fail((len + 1) > len,                                                            \
                                    libiodef_error_verbose(LIBIODEF_ERROR_INVAL_LENGTH,                          \
                                                          "string length warning: wrap around would occur"));  \
                                                                                                               \
        libiodef_return_val_if_fail(str[len] == 0,                                                              \
                                   libiodef_error_verbose(LIBIODEF_ERROR_STRING_NOT_NULL_TERMINATED,             \
                                                         "string warning: not nul terminated"));               \
} while(0)



struct libiodef_string {
...
static int string_buf_alloc(libiodef_string_t *string, size_t len)
{
        /*
...
static void string_buf_copy(libiodef_string_t *string, const char *buf, size_t len)
{
...
static int make_string_own(libiodef_string_t *s, size_t len)
{
        char *ptr;
        ...
        s->size = len;
        s->data.rwbuf = ptr;
        s->flags |= LIBIODEF_STRING_OWN_DATA;
...
static int allocate_more_chunk_if_needed(libiodef_string_t *s, size_t needed_len)
{
        int ret;
        char *ptr;
        size_t len;
        ...
        if ( s->size + len < s->size )
                return libiodef_error(LIBIODEF_ERROR_INVAL_LENGTH);

        if ( s->flags & LIBIODEF_STRING_OWN_DATA ) {
                ptr = _libiodef_realloc(s->data.rwbuf, s->size + len);
                if ( ! ptr )
                        return libiodef_error_from_errno(errno);
...
/**
 * libiodef_string_new:
 * @string: Pointer where to store the created #libiodef_string_t.
 *
 * Create a new #libiodef_string_t object, and store in in @string.
 *
...
int libiodef_string_new(libiodef_string_t **string)
{
        *string = calloc(1, sizeof(**string));
        ...
        (*string)->refcount = 1;
        libiodef_list_init(&(*string)->_list);
        (*string)->flags = LIBIODEF_STRING_OWN_STRUCTURE;
...
/**
 * libiodef_string_ref:
 * @string: Pointer to a #libiodef_string_t object to reference.
 *
...
{
        libiodef_return_val_if_fail(string, NULL);
...
/**
 * libiodef_string_new_dup_fast:
 * @string: Pointer where to store the created #libiodef_string_t object.
 * @str: Initial string value.
 * @len: Lenght of @str.
 *
 * Create a new #libiodef_string_t object with a copy of @str as it's
 * initial value.  The copy is owned by the @string and will be freed
 * upon libiodef_string_destroy().
 *
...
int libiodef_string_new_dup_fast(libiodef_string_t **string, const char *str, size_t len)
{
        ...
        libiodef_return_val_if_fail(str, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        STRING_RETURN_IF_INVALID(str, len);

        ret = libiodef_string_new(string);
        if ( ret < 0 )
                return ret;
        ...
        (*string)->flags |= LIBIODEF_STRING_OWN_DATA;
...
{
        libiodef_return_val_if_fail(str, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        return libiodef_string_new_dup_fast(string, str, strlen(str));
}



/**
 * libiodef_string_new_nodup_fast:
 * @string: Pointer where to store the created #libiodef_string_t object.
 * @str: Initial string value.
 * @len: Lenght of @str.
 *
 * Create a new #libiodef_string_t object with a reference to @str as
 * initial value.  @str is owned by @string and will be freed upon
 * libiodef_string_destroy().
 *
 * Returns: 0 on success, a negative value if an error occured.
 */
...
{
        libiodef_return_val_if_fail(str, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        return libiodef_string_new_nodup_fast(string, str, strlen(str));
}



/**
 * libiodef_string_new_ref_fast:
 * @string: Pointer where to store the created #libiodef_string_t object.
 * @str: Initial string value.
 * @len: Length of @str.
 *
 * Create a new #libiodef_string_t object with a reference to @str as
 * initial value. @str won't be freed upon libiodef_string_destroy().
 *
 * Returns: 0 on success, a negative value if an error occured.
 */
int libiodef_string_new_ref_fast(libiodef_string_t **string, const char *buf, size_t len)
{
        int ret;
...
{
        libiodef_return_val_if_fail(str, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        return libiodef_string_new_ref_fast(string, str, strlen(str));
}



/**
 * libiodef_string_set_dup_fast:
 * @string: Pointer to a #libiodef_string_t object.
 * @buf: String to store in @string.
 * @len: Lenght of @buf.
 *
 * Store a copy of the string pointed by @buf in @string.
 * The @buf copy will be freed upon libiodef_string_destroy().
 *
 * Returns: 0 on success, or a negative value if an error occured.
 */
int libiodef_string_set_dup_fast(libiodef_string_t *string, const char *buf, size_t len)
{
        ...
        libiodef_return_val_if_fail(string, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        libiodef_return_val_if_fail(buf, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        STRING_RETURN_IF_INVALID(buf, len);

        libiodef_string_destroy_internal(string);
...
int libiodef_string_set_dup(libiodef_string_t *string, const char *buf)
{
        libiodef_return_val_if_fail(string, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        libiodef_return_val_if_fail(buf, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        return libiodef_string_set_dup_fast(string, buf, strlen(buf));
}



/**
 * libiodef_string_set_nodup_fast:
 * @string: Pointer to a #libiodef_string_t object.
 * @buf: String to store in @string.
 * @len: Lenght of @buf.
 *
 * Store a reference to the string pointed by @buf in @string.
 * The referenced @buf will be freed upon libiodef_string_destroy().
 *
 * Returns: 0 on success, or a negative value if an error occured.
 */
...
int libiodef_string_set_nodup(libiodef_string_t *string, char *buf)
{
        ...
        return libiodef_string_set_nodup_fast(string, buf, strlen(buf));
}



/**
 * libiodef_string_set_ref_fast:
 * @string: Pointer to a #libiodef_string_t object.
 * @buf: String to store in @string.
 * @len: Lenght of @buf.
 *
...
int libiodef_string_set_ref(libiodef_string_t *string, const char *buf)
{
        ...
        return libiodef_string_set_ref_fast(string, buf, strlen(buf));
}
...
/**
 * libiodef_string_copy_ref:
 * @src: Pointer to a #libiodef_string_t object to copy data from.
 * @dst: Pointer to a #libiodef_string_t object to copy data to.
 *
...
int libiodef_string_copy_ref(const libiodef_string_t *src, libiodef_string_t *dst)
{
        ...
        libiodef_string_destroy_internal(dst);

        dst->size = src->size;
        dst->index = src->index;
        dst->data.robuf = src->data.robuf;
        dst->flags &= ~LIBIODEF_STRING_OWN_DATA;
...
/**
 * libiodef_string_clone:
 * @src: Pointer to an existing #libiodef_string_t object.
 * @dst: Pointer to an address where to store the created #libiodef_string_t object.
 *
 * Clone @src within a new #libiodef_string_t object stored into @dst.
...
{
        libiodef_return_val_if_fail(string, 0);
        return string->index;
}
...
/**
 * libiodef_string_get_string_or_default:
 * @string: Pointer to a #libiodef_string_t object.
...
/**
 * libiodef_string_get_string_released:
 * @string: Pointer to a #libiodef_string_t object.
...
int libiodef_string_get_string_released(libiodef_string_t *string, char **outptr)
{
        ...
        *outptr = NULL;
        ...
        if ( ! (string->flags & LIBIODEF_STRING_OWN_DATA) ) {
                *outptr = strdup(string->data.robuf);
                return (*outptr) ? 0 : libiodef_error_from_errno(errno);
        }

        if ( string->index + 1 <= string->index )
                return libiodef_error(LIBIODEF_ERROR_INVAL_LENGTH);

        *outptr = _libiodef_realloc(string->data.rwbuf, string->index + 1);
        if ( ! *outptr )
                return libiodef_error_from_errno(errno);
...
{
        libiodef_return_val_if_fail(string, TRUE);
...
{
        libiodef_return_if_fail(string);

        if ( (string->flags & LIBIODEF_STRING_OWN_DATA) && string->data.rwbuf )
...
void libiodef_string_destroy(libiodef_string_t *string)
{
        ...
        if ( ! libiodef_list_is_empty(&string->_list) )
                libiodef_list_del_init(&string->_list);

        libiodef_string_destroy_internal(string);

        if ( string->flags & LIBIODEF_STRING_OWN_STRUCTURE )
                free(string);
}
...
int libiodef_string_vprintf(libiodef_string_t *string, const char *fmt, va_list ap)
{
        ...
        libiodef_return_val_if_fail(string, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        libiodef_return_val_if_fail(fmt, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        ...
        ret = libiodef_string_vprintf(string, fmt, bkp);
...
int libiodef_string_sprintf(libiodef_string_t *string, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        ret = libiodef_string_vprintf(string, fmt, ap);
        va_end(ap);
...
/**
 * libiodef_string_cat:
 * @dst: Pointer to a #libiodef_string_t object.
 * @str: Pointer to a string.
 * @len: Length of @str to copy.
 *
 * The libiodef_string_ncat() function appends @len characters from @str to
 * the @dst #libiodef_string_t object over-writing the `\0' character at the
...
int libiodef_string_ncat(libiodef_string_t *dst, const char *str, size_t len)
{
        ...
        if ( dst->flags & LIBIODEF_STRING_OWN_DATA && len < (dst->size - dst->index) ) {
        ...
        return libiodef_string_ncat(dst, str, len);
}
 ...
 * @str: Pointer to a string.
 *
 * The libiodef_string_cat() function appends the @str string to the @dst
 * libiodef_string_t object over-writing the `\0' character at the end of
...
int libiodef_string_cat(libiodef_string_t *dst, const char *str)
{
        ...
        return libiodef_string_ncat(dst, str, strlen(str));
}
...
int libiodef_string_truncate(libiodef_string_t *string, size_t len)
{
        ...
        libiodef_return_val_if_fail(string, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        libiodef_return_val_if_fail(len <= string->index, libiodef_error(LIBIODEF_ERROR_ASSERTION));

        if ( len && ! (string->flags & LIBIODEF_STRING_OWN_DATA) ) {
...
{
        libiodef_string_truncate(string, 0);
}
...
/**
 * libiodef_string_compare:
 * @str1: Pointer to a #libiodef_string_t object to compare with @str2.
 * @str2: Pointer to a #libiodef_string_t object to compare with @str1.
 *
