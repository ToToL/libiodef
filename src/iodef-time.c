...
static char *parse_time_ymd(struct tm *tm, const char *buf)
{
        ...
        /*
         * The IODEF draft only permits the 'T' variant here
         */
...
static int digit2usec(uint32_t n, int digit_count)
{
        ...
        if ( digit_count > max_digit )
                return libiodef_error_verbose(LIBIODEF_ERROR_GENERIC, "Invalid number of digits for time fraction");
...
static int parse_time_hmsu(struct tm *tm, uint32_t *usec, char **buf)
{
    ...
    fmterror:
        return libiodef_error_verbose(LIBIODEF_ERROR_GENERIC, "error parsing time field, format should be: HH:MM:SS(.fraction)");
}
...
/**
 * iodef_time_set_from_string:
 * @time: Pointer to an #iodef_time_t object.
 * @buf: Pointer to a string describing a time in an IODEF conforming format.
 *
 ...
 * to the IODEF definition (v. 0.10, section 3.2.6).
 *
 * Additionally, the provided time might be separated with white spaces,
 * instead of the IODEF defined 'T' character.
 *
...
int iodef_time_set_from_string(iodef_time_t *time, const char *buf)
{
        ...
        char *ptr;
        struct tm tm;
        libiodef_bool_t miss_gmt = FALSE;
        ...
        if ( ! ptr )
                return libiodef_error_verbose(LIBIODEF_ERROR_GENERIC, "error parsing date field, format should be: YY-MM-DD");
        ...
        if ( *ptr ) {
                ...
                if ( *ptr ) {
                        ...
                        if ( ret < 0 )
                                return libiodef_error_verbose(LIBIODEF_ERROR_GENERIC, "error parsing GMT offset field (Z)?(+|-)?HH:MM");
        ...
        if ( miss_gmt ) {
                long gmtoff;
                libiodef_get_gmt_offset_from_tm(&tm, &gmtoff);
                time->gmt_offset = (int32_t) gmtoff;
        }

        time->sec = miss_gmt ? mktime(&tm) : libiodef_timegm(&tm);
        return 0;
}
...
/**
 * iodef_time_new_from_string:
 * @time: Address where to store the created #iodef_time_t object.
 * @buf: Pointer to a string describing a time in an IODEF conforming format.
 *
 * Creates an #iodef_time_t object filled with information retrieved
 * from the user provided @buf, containing a string describing a time in a format
 * conforming to the IODEF definition  (v. 0.10, section 3.2.6).
 *
 * Additionally, the provided time might be separated with white spaces, instead
 * of the IODEF define 'T' character. The format might not specify a timezone
 * (will assume UTC in this case).
 *
 * The resulting #iodef_time_t object is stored in @time.
 *
 * Returns: 0 on success, a negative value if an error occured.
 */
int iodef_time_new_from_string(iodef_time_t **time, const char *buf)
{
        ...
        ret = iodef_time_new(time);
        if ( ret < 0 )
                return ret;

        ret = iodef_time_set_from_string(*time, buf);
        if ( ret < 0 ) {
                free(*time);
                return ret;
        }
...
/**
 * iodef_time_to_ntpstamp:
 * @time: Pointer to an IODEF time structure.
 * @out: Pointer to a #libiodef_string_t output buffer.
 *
 * Translates @time to an user readable NTP timestamp string,
 * conforming to the IODEF defined time format.
 *
...
int iodef_time_to_ntpstamp(const iodef_time_t *time, libiodef_string_t *out)
{
        ...
        tv.tv_sec = iodef_time_get_sec(time);
        tv.tv_usec = iodef_time_get_usec(time);
        ...
        ret = libiodef_string_sprintf(out, "0x%08lx.0x%08lx", (unsigned long) ts.l_ui, (unsigned long) ts.l_uf);
...
/**
 * iodef_time_to_string:
 * @time: Pointer to an IODEF time structure.
 * @out: Pointer to a #libiodef_string_t output buffer.
 *
 * Translates @time to an user readable string conforming to the IODEF
 * defined time format.
 *
 * Returns: number of bytes written on success, a negative value if an error occured.
 */
int iodef_time_to_string(const iodef_time_t *time, libiodef_string_t *out)
{
        ...
        return libiodef_string_sprintf(out, "%d-%.2d-%.2dT%.2d:%.2d:%.2d.%02u%+.2d:%.2d",
                                      utc.tm_year + 1900, utc.tm_mon + 1, utc.tm_mday,
                                      utc.tm_hour, utc.tm_min, utc.tm_sec, iodef_time_get_usec(time),
                                      hour_off, min_off);
}
...
/**
 * iodef_time_new_from_ntpstamp:
 * @time: Address where to store the created #iodef_time_t object.
 * @buf: Pointer to a string containing an NTP timestamp.
 *
 * Creates an #iodef_time_t object filled with information provided
...
int iodef_time_new_from_ntpstamp(iodef_time_t **time, const char *buf)
{
        ...
        ret = iodef_time_set_from_ntpstamp(*time, buf);
        if ( ret < 0 ) {
                free(*time);
                return ret;
        }
...
int iodef_time_set_from_timeval(iodef_time_t *time, const struct timeval *tv)
{
        ...
        libiodef_return_val_if_fail(time, libiodef_error(LIBIODEF_ERROR_ASSERTION));
        libiodef_return_val_if_fail(tv, libiodef_error(LIBIODEF_ERROR_ASSERTION));

        ret = libiodef_get_gmt_offset_from_time((const time_t *) &tv->tv_sec, &gmtoff);
        if ( ret < 0 )
                return ret;
...
int iodef_time_new_from_timeval(iodef_time_t **time, const struct timeval *tv)
{
        ...
        return iodef_time_set_from_timeval(*time, tv);
}



/**
 * iodef_time_set_from_gettimeofday:
 * @time: Pointer to an #iodef_time_t object.
 *
...
/**
 * iodef_time_new_from_gettimeofday:
 * @time: Address where to store the created #iodef_time_t object.
 *
 * Creates an #iodef_time_t object filled with information retrieved
...
int iodef_time_new_from_gettimeofday(iodef_time_t **time)
{
        ...
        return iodef_time_new_from_timeval(time, &tv);
}
 ...
 * iodef_time_destroy() won't destroy @time until the refcount
 * reach 0.
 *
...
{
        libiodef_return_val_if_fail(time, NULL);
 ...
 * @time: Address where to store the created #iodef_time_t object.
 *
 * Creates an empty #iodef_time_t object and store it in @time.
 *
 * Returns: 0 on success, a negative value if an error occured.
 */
...
/**
 * iodef_time_clone:
 * @src: Pointer to a #iodef_time_t to clone.
...
int iodef_time_clone(const iodef_time_t *src, iodef_time_t **dst)
{
        ...
        ret = iodef_time_new(dst);
        if ( ret < 0 )
                return ret;

        return iodef_time_copy(src, *dst);
}



/**
 * iodef_time_set_from_time:
 * @time: Pointer to an #iodef_time_t object.
...
void iodef_time_set_from_time(iodef_time_t *time, const time_t *t)
{
        ...
        libiodef_return_if_fail(time);
        libiodef_return_if_fail(t);

        libiodef_get_gmt_offset_from_time(t, &gmtoff);
...
/**
 * iodef_time_new_from_time:
 * @time: Address where to store the created #iodef_time_t object.
 * @t: Pointer to a time_t.
 *
 * Creates a new #iodef_time_t object and store it in @time.
...
int iodef_time_new_from_time(iodef_time_t **time, const time_t *t)
{
        ...
        libiodef_return_val_if_fail(t, libiodef_error(LIBIODEF_ERROR_ASSERTION));

        ret = iodef_time_new(time);
        if ( ret < 0 )
                return ret;

        iodef_time_set_from_time(*time, t);
...
/**
 * iodef_time_set_gmt_offset:
 * @time: Pointer to a #iodef_time_t.
 ...
 * set @time current time also requires the use of iodef_time_set_sec()
 * and iodef_time_set_usec().
 */
 ...
 *
 * WARNING: this is just an accessor function, and using it to
 * set @time current time also requires the use of iodef_time_set_usec()
 * and iodef_time_set_gmt_offset().
 */
...
{
        libiodef_return_val_if_fail(time, 0);
...
/**
 * iodef_time_copy:
 * @src: Pointer to a #iodef_time_t to copy data from.
 * @dst: Pointer to a #iodef_time_t to copy data to.
 *
...
/**
 * iodef_time_compare:
 * @time1: Pointer to an #iodef_time_t object to compare with @time2.
 * @time2: Pointer to an #iodef_time_t object to compare with @time1.
 *
