...
static int integer_compare(time_t matched, time_t wanted, iodef_criterion_operator_t op)
{
        if ( op & IODEF_CRITERION_OPERATOR_EQUAL && matched == wanted )
                return 1;

        if ( op & IODEF_CRITERION_OPERATOR_LESSER && matched < wanted)
                return 1;

        if ( op & IODEF_CRITERION_OPERATOR_GREATER && matched > wanted )
                return 1;
...
static int btime_match(const iodef_criterion_value_t *cv, iodef_criterion_operator_t operator, iodef_value_t *value)
{
        int ret;
        ...
        if ( operator & IODEF_CRITERION_OPERATOR_NOT )
                return (ret == 1) ? 0 : 1;
        else
                return (ret == 1) ? 1 : 0;
}
...
static int btime_clone(const iodef_criterion_value_t *src, iodef_criterion_value_t *dst)
{
        /*
         * The API does not allow to change the value of an existing
         * iodef_criterion_value_t object, so we can simply regerence the source.
         */
...
static void btime_destroy(iodef_criterion_value_t *cv)
{
 ...
 * regex stuff
 */
static int regex_match(const iodef_criterion_value_t *cv, iodef_criterion_operator_t operator, iodef_value_t *value)
{
        int ret;
        const char *str = NULL;
        ...
        if ( iodef_value_get_type(value) == IODEF_VALUE_TYPE_STRING )
                ...
                str = iodef_class_enum_to_string(class, iodef_value_get_enum(value));
        }

        else if ( iodef_value_get_type(value) == IODEF_VALUE_TYPE_DATA ) {
                iodef_data_t *data;

                data = iodef_value_get_data(value);
                if ( iodef_data_get_type(data) == IODEF_DATA_TYPE_CHAR_STRING )
                        str = iodef_data_get_data(data);
        }
...
static int regex_clone(const iodef_criterion_value_t *src, iodef_criterion_value_t *dst)
{
        /*
         * The API does not allow to change the value of an existing
         * iodef_criterion_value_t object, so we can simply regerence the source.
...
static void regex_destroy(iodef_criterion_value_t *cv)
{
 ...
 * value stuff
 */
static int value_match(const iodef_criterion_value_t *cv, iodef_criterion_operator_t operator, iodef_value_t *value)
{
        return iodef_value_match(value, cv->value, operator);
}
...
static int value_clone(const iodef_criterion_value_t *cv, iodef_criterion_value_t *dst)
{
        int ret;
        iodef_value_t *res;

        ret = iodef_value_clone(cv->value, &res);
        dst->value = res;
...
static void value_destroy(iodef_criterion_value_t *cv)
{
        iodef_value_destroy(cv->value);
}
 ...
 *
 */
static int do_match_cb(iodef_value_t *value, void *extra)
{
        int ret;
        struct match_cb *mcb = extra;
        iodef_criterion_value_t *cv = mcb->cv;
        iodef_criterion_operator_t operator = mcb->operator;

        if ( ! cv )
                ret = (operator == IODEF_CRITERION_OPERATOR_NOT_NULL) ? (value != NULL) : (value == NULL);

        else {
                if ( value && iodef_value_is_list(value) )
                        return iodef_value_iterate(value, do_match_cb, mcb);
...
int iodef_criterion_value_clone(const iodef_criterion_value_t *src, iodef_criterion_value_t **dst)
{
        ...
        ret = iodef_criterion_value_new(dst);
        if ( ret < 0 )
                return ret;
...
static int my_value_match(iodef_value_t *value, struct match_cb *mcb)
{
        if ( value )
                return iodef_value_iterate(value, do_match_cb, mcb);
        else
...
int iodef_criterion_value_match(iodef_criterion_value_t *cv, iodef_value_t *value,
                                iodef_criterion_operator_t op)
{
        ...
        if ( value && iodef_value_is_list(value) && op != IODEF_CRITERION_OPERATOR_NOT_NULL ) {
                if ( op == IODEF_CRITERION_OPERATOR_NULL )
                        return (mcb.nmatch == 0) ? 1 : 0;

                if ( op & IODEF_CRITERION_OPERATOR_NOT && mcb.nmatch > 0 )
                        return 0;
        }
...
int iodef_criterion_value_new_broken_down_time(iodef_criterion_value_t **cv, const char *time, iodef_criterion_operator_t op)
{
        ...
        ret = iodef_criterion_value_new(cv);
        if ( ret < 0 )
                return ret;
        ...
        (*cv)->type = IODEF_CRITERION_VALUE_TYPE_BROKEN_DOWN_TIME;
...
int iodef_criterion_value_new_regex(iodef_criterion_value_t **cv, const char *regex, iodef_criterion_operator_t op)
{
        ...
        (*cv)->type = IODEF_CRITERION_VALUE_TYPE_REGEX;
...
int iodef_criterion_value_new_value(iodef_criterion_value_t **cv,
                                    iodef_value_t *value, iodef_criterion_operator_t op)
{
        int ret;

        ret = iodef_value_check_operator(value, op);
        if ( ret < 0 )
                return ret;
        ...
        (*cv)->type = IODEF_CRITERION_VALUE_TYPE_VALUE;
...
int iodef_criterion_value_new_from_string(iodef_criterion_value_t **cv,
                                          iodef_path_t *path, const char *value,
                                          iodef_criterion_operator_t operator)
{
        int ret;
        iodef_value_t *val;
        ...
        tid = iodef_path_get_value_type(path, -1);

        if ( tid == IODEF_VALUE_TYPE_TIME ) {
                ret = iodef_criterion_value_new_broken_down_time(cv, value, operator);
                if ( ret == 0 )
                        return ret;
        }

        else if ( operator & IODEF_CRITERION_OPERATOR_REGEX &&
                  (tid == IODEF_VALUE_TYPE_STRING || tid == IODEF_VALUE_TYPE_ENUM || tid == IODEF_VALUE_TYPE_DATA) )
                return iodef_criterion_value_new_regex(cv, value, operator);

        /*
         * It's more understandable for the user if we check the operator
         * prior to checking the value.
         */
        ret = iodef_value_type_check_operator(tid, operator);
        if ( ret < 0 )
                return ret;

        if ( tid == IODEF_VALUE_TYPE_ENUM && operator & IODEF_CRITERION_OPERATOR_SUBSTR )
                ret = iodef_value_new_from_string(&val, IODEF_VALUE_TYPE_STRING, value);
        else
        ...
        ret = iodef_criterion_value_new_value(cv, val, operator);
        if ( ret < 0 ) {
...
{
        return cv->type == IODEF_CRITERION_VALUE_TYPE_VALUE ? cv->value : NULL;
}
...
{
        return cv->type == IODEF_CRITERION_VALUE_TYPE_BROKEN_DOWN_TIME ? cv->value : NULL;
}
...
{
        return cv->type == IODEF_CRITERION_VALUE_TYPE_REGEX ? ((struct regex_value *)cv->value)->regex_string : NULL;
}
