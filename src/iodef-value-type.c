...
#define CLASS_OPERATOR  IODEF_CRITERION_OPERATOR_NULL|IODEF_CRITERION_OPERATOR_NOT| \
                        IODEF_CRITERION_OPERATOR_EQUAL

#define DATA_OPERATOR    IODEF_CRITERION_OPERATOR_EQUAL|IODEF_CRITERION_OPERATOR_NOT| \
                         IODEF_CRITERION_OPERATOR_LESSER|IODEF_CRITERION_OPERATOR_GREATER|IODEF_CRITERION_OPERATOR_SUBSTR

#define TIME_OPERATOR    IODEF_CRITERION_OPERATOR_LESSER|IODEF_CRITERION_OPERATOR_GREATER| \
                         IODEF_CRITERION_OPERATOR_EQUAL|IODEF_CRITERION_OPERATOR_NOT

#define STRING_OPERATOR  IODEF_CRITERION_OPERATOR_SUBSTR|IODEF_CRITERION_OPERATOR_EQUAL| \
                         IODEF_CRITERION_OPERATOR_NOT|IODEF_CRITERION_OPERATOR_NOCASE

#define INTEGER_OPERATOR IODEF_CRITERION_OPERATOR_LESSER|IODEF_CRITERION_OPERATOR_GREATER|\
                         IODEF_CRITERION_OPERATOR_EQUAL|IODEF_CRITERION_OPERATOR_NOT
...
#define GENERIC_ONE_BASE_RW_FUNC(scanfmt, printfmt, name, type)                            \
        static int name ## _read(iodef_value_type_t *dst, const char *buf)                 \
        {                                                                                  \
                char *endptr;                                                              \
                                                                                           \
...
typedef struct {
        ...
        int (*copy)(const iodef_value_type_t *src, void *dst, size_t size);
        int (*clone)(const iodef_value_type_t *src, iodef_value_type_t *dst, size_t size);
        int (*ref)(const iodef_value_type_t *src);

        void (*destroy)(iodef_value_type_t *type);
 ...
 * generic functions.
 */
static int charstring_compare(const char *s1, const char *s2, iodef_criterion_operator_t op)
{
        if ( (s1 && !s2) || (s2 && !s1) )
                return -1;

        if ( op == (IODEF_CRITERION_OPERATOR_EQUAL|IODEF_CRITERION_OPERATOR_NOCASE) && strcasecmp(s1, s2) == 0 )
                return 0;

        else if ( op == IODEF_CRITERION_OPERATOR_EQUAL && strcmp(s1, s2) == 0 )
                return 0;

        else if ( op == (IODEF_CRITERION_OPERATOR_SUBSTR|IODEF_CRITERION_OPERATOR_NOCASE) && strcasestr(s1, s2) )
                return 0;

        else if ( op == IODEF_CRITERION_OPERATOR_SUBSTR && strstr(s1, s2) )
                return 0;
...
static int generic_copy(const iodef_value_type_t *src, void *dst, size_t size)
{
...
static int generic_clone(const iodef_value_type_t *src, iodef_value_type_t *dst, size_t size)
{
...
static int generic_compare(const iodef_value_type_t *t1, const iodef_value_type_t *t2,
                           size_t size, iodef_criterion_operator_t op)
{
        ...
        if ( ret == 0 && op & IODEF_CRITERION_OPERATOR_EQUAL )
                return 0;

        if ( ret < 0 && op & IODEF_CRITERION_OPERATOR_LESSER )
                return 0;

        if ( ret > 0 && op & IODEF_CRITERION_OPERATOR_GREATER )
                return 0;
 ...
 * Enum specific
 */
static int enum_copy(const iodef_value_type_t *src, void *dst, size_t size)
{
...
static int enum_read(iodef_value_type_t *dst, const char *buf)
{
        int ret;
 ...
 */
static int time_compare(const iodef_value_type_t *t1, const iodef_value_type_t *t2,
                        size_t size, iodef_criterion_operator_t op)
{
        int ret;
        ...
        ret = iodef_time_compare(t1->data.time_val, t2->data.time_val);
        if ( op & IODEF_CRITERION_OPERATOR_EQUAL && ret == 0 )
                return 0;

        else if ( op & IODEF_CRITERION_OPERATOR_LESSER && ret < 0 )
                return 0;

        else if ( op & IODEF_CRITERION_OPERATOR_GREATER && ret > 0 )
                return 0;
...
static int time_read(iodef_value_type_t *dst, const char *buf)
{
        int ret;

        ret = iodef_time_new_from_ntpstamp(&dst->data.time_val, buf);
        if ( ret == 0 )
                return 0;

        ret = iodef_time_new_from_string(&dst->data.time_val, buf);
        if ( ret == 0 )
                return 0;
...
static int time_copy(const iodef_value_type_t *src, void *dst, size_t size)
{
        return iodef_time_copy(src->data.time_val, dst);
}



static int time_clone(const iodef_value_type_t *src, iodef_value_type_t *dst, size_t size)
{
        return iodef_time_clone(src->data.time_val, &dst->data.time_val);
}


static int time_ref(const iodef_value_type_t *src)
{
        iodef_time_ref(src->data.time_val);
        return 0;
}


static void time_destroy(iodef_value_type_t *type)
{
        iodef_time_destroy(type->data.time_val);
}
 ...
 */
static int data_compare(const iodef_value_type_t *t1, const iodef_value_type_t *t2,
                        size_t len, iodef_criterion_operator_t op)
{
        int ret;
        ...
        if ( t1 && t1->data.data_val )
                s1 = iodef_data_get_data(t1->data.data_val);

        if ( t2 && t2->data.string_val )
                s2 = iodef_data_get_data(t2->data.data_val);

        if ( (s1 && !s2) || (s2 && !s1) )
                return -1;

        if ( op & IODEF_CRITERION_OPERATOR_SUBSTR ) {
                s1_len = iodef_data_get_len(t1->data.data_val);
                s2_len = iodef_data_get_len(t2->data.data_val);
                return ( memmem(s1, s1_len, s2, s2_len) ) ? 0 : -1;
        }

        ret = iodef_data_compare(t1->data.data_val, t2->data.data_val);
        if ( ret == 0 && op & IODEF_CRITERION_OPERATOR_EQUAL )
                return 0;

        else if ( ret < 0 && op & IODEF_CRITERION_OPERATOR_LESSER )
                return 0;

        else if ( ret > 0 && op & IODEF_CRITERION_OPERATOR_GREATER )
                return 0;
...
static int data_read(iodef_value_type_t *dst, const char *src)
{
        return iodef_data_new_char_string_dup_fast(&dst->data.data_val, src, strlen(src));
}
...
static int data_copy(const iodef_value_type_t *src, void *dst, size_t size)
{
        return iodef_data_copy_dup(src->data.data_val, dst);
}



static int data_clone(const iodef_value_type_t *src, iodef_value_type_t *dst, size_t size)
{
        return iodef_data_clone(src->data.data_val, &dst->data.data_val);
}


static int data_ref(const iodef_value_type_t *src)
{
        iodef_data_ref(src->data.data_val);
        return 0;
}


static void data_destroy(iodef_value_type_t *type)
{
        iodef_data_destroy(type->data.data_val);
}
 ...
 * class specific
 */
static int class_compare(const iodef_value_type_t *c1,
                         const iodef_value_type_t *c2, size_t len, iodef_criterion_operator_t op)
{
        if ( (c1 && !c2) || (c2 && !c1) )
                return -1;

        return iodef_class_compare(c1->data.class_val.class_id,
                                   c1->data.class_val.object, c2->data.class_val.object);
}


static int class_copy(const iodef_value_type_t *src, void *dst, size_t size)
{
        return iodef_class_copy(src->data.class_val.class_id, src->data.class_val.object, dst);
}


static int class_clone(const iodef_value_type_t *src, iodef_value_type_t *dst, size_t size)
{
        dst->data.class_val.class_id = src->data.class_val.class_id;
        return iodef_class_clone(src->data.class_val.class_id, src->data.class_val.object, &dst->data.class_val.object);
}


static int class_ref(const iodef_value_type_t *src)
{
        return iodef_class_ref(src->data.class_val.class_id, src->data.class_val.object);
}


static void class_destroy(iodef_value_type_t *type)
{
        iodef_class_destroy(type->data.class_val.class_id, type->data.class_val.object);
}
...
static int class_to_string(iodef_class_id_t parent_class, void *parent, prelude_string_t *out)
{
        ...
        for ( i = 0; ; i++ ) {
                ret = iodef_class_get_child(parent, parent_class, i, &childptr);
                if ( ret < 0 )
                        break;
                ...
                vtype = iodef_class_get_child_value_type(parent_class, i);
                childclass = iodef_class_get_child_class(parent_class, i);

                if ( vtype != IODEF_VALUE_TYPE_CLASS ) {
                        ...
                        if ( ! iodef_class_is_child_list(parent_class, i) ) {
                                ...
                                ret = iodef_value_to_string(childptr, out);
                                iodef_value_destroy(childptr);
                                if ( ret < 0 )
                                        return ret;
                        }
...
static const iodef_value_type_operation_t ops_tbl[] = {
        ...
        { "enum", sizeof(iodef_value_type_enum_t), ENUM_OPERATOR, enum_copy,
...
int iodef_value_type_ref(const iodef_value_type_t *vt)
{
        int ret;
...
int iodef_value_type_compare(const iodef_value_type_t *type1,
                             const iodef_value_type_t *type2,
                             iodef_criterion_operator_t op)
{
        ...
        ret = ops_tbl[tid].compare(type1, type2, ops_tbl[tid].len, op & ~IODEF_CRITERION_OPERATOR_NOT);
        if ( ret < 0 ) /* not an error -> no match */
                ret = 1;

        if ( op & IODEF_CRITERION_OPERATOR_NOT )
                return (ret == 0) ? 1 : 0;
        else
                return ret;
}
