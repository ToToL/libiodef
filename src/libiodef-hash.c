...
struct libiodef_hash {
        size_t lists_size;
        libiodef_list_t *lists;
...
static unsigned int hash_value_calc(libiodef_hash_t *hash, const void *key)
{
...
static hash_elem_t *hash_elem_get(libiodef_hash_t *hash, const void *key)
{
        libiodef_list_t *list;
        libiodef_list_t *ptr;
        ...
        libiodef_list_for_each(list, ptr) {
                hash_elem = libiodef_list_entry(ptr, hash_elem_t, list);
...
static  void hash_elem_key_destroy(libiodef_hash_t *hash, hash_elem_t *hash_elem)
{
...
static void hash_elem_value_destroy(libiodef_hash_t *hash, hash_elem_t *hash_elem)
{
...
int libiodef_hash_new(libiodef_hash_t **nhash,
...
{
        return libiodef_hash_new2(nhash, HASH_DEFAULT_SIZE, hash_func, key_cmp_func, key_destroy_func, value_destroy_func);
}



int libiodef_hash_new2(libiodef_hash_t **nhash, size_t size,
                      unsigned int (*hash_func)(const void *),
                      int (*key_cmp_func)(const void *, const void *),
                      void (*key_destroy_func)(void *),
                      void (*value_destroy_func)(void *))
{
        ...
        *nhash = hash = calloc(1, sizeof (*hash));
        ...
        for ( i = 0; i < hash->lists_size; i++ )
                libiodef_list_init(hash->lists + i);
...
void libiodef_hash_destroy(libiodef_hash_t *hash)
{
        size_t cnt;
        libiodef_list_t *list;
        libiodef_list_t *ptr;
...
int libiodef_hash_add(libiodef_hash_t *hash, void *key, void *value)
{
        hash_elem_t *hash_elem;
        libiodef_list_t *list;
        ...
        list = hash->lists + hash_value_calc(hash, key);
        libiodef_list_add(list, &hash_elem->list);
...
int libiodef_hash_set(libiodef_hash_t *hash, void *key, void *value)
{
        ...
        return libiodef_hash_add(hash, key, value);
}



void *libiodef_hash_get(libiodef_hash_t *hash, const void *key)
{
        hash_elem_t *hash_elem;
...
int libiodef_hash_elem_destroy(libiodef_hash_t *hash, const void *key)
{
        hash_elem_t *hash_elem;
...
void libiodef_hash_iterate(libiodef_hash_t *hash, void (*cb)(void *data))
{
        unsigned int i;
        libiodef_list_t *tmp;
        hash_elem_t *hash_elem;
        ...
        for ( i = 0; i < hash->lists_size; i++ ) {
                libiodef_list_for_each(&hash->lists[i], tmp) {
                        hash_elem = libiodef_list_entry(tmp, hash_elem_t, list);
                        cb(hash_elem->value);
                }
        }
}
