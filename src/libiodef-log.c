...
static int do_log_print(libiodef_log_t level, const char *str);


static int debug_level = 0;
static int raise_abort_level = 0;
libiodef_bool_t raise_abort_set = FALSE;
static int log_level = LIBIODEF_LOG_INFO;

static char *global_prefix = NULL;

static FILE *debug_logfile = NULL;
static libiodef_log_flags_t log_flags = 0;
static int (*global_log_cb)(libiodef_log_t level, const char *str) = do_log_print;
static void (*external_log_cb)(libiodef_log_t level, const char *str) = NULL;
...
static int do_log_syslog(libiodef_log_t level, const char *str)
{
        int slevel = LOG_INFO;

        if ( level == LIBIODEF_LOG_CRIT )
                slevel = LOG_CRIT;

        else if ( level == LIBIODEF_LOG_ERR )
                slevel = LOG_ERR;

        else if ( level == LIBIODEF_LOG_WARN )
                slevel = LOG_WARNING;

        else if ( level == LIBIODEF_LOG_INFO )
                slevel = LOG_INFO;

        else if ( level >= LIBIODEF_LOG_DEBUG )
                slevel = LOG_DEBUG;
...
static inline FILE *get_out_fd(libiodef_log_t level)
{
        return (level < LIBIODEF_LOG_INFO) ? stderr : stdout;
}
...
static int do_log_external(libiodef_log_t level, const char *str)
{
...
static inline libiodef_bool_t need_to_log(libiodef_log_t level, libiodef_log_t cur)
{
        return (level > cur) ? FALSE : TRUE;
}


static const char *level_to_string(libiodef_log_t level)
{
        if ( level >= LIBIODEF_LOG_DEBUG )
                return "DEBUG";
        ...
        else if ( level == LIBIODEF_LOG_CRIT )
                return "CRITICAL";
...
static ssize_t get_header(libiodef_log_t level, char *buf, size_t size)
{
        int ret;
        struct tm *t;
        size_t len = 0;
        ...
        if ( ! (log_flags & LIBIODEF_LOG_FLAGS_SYSLOG) ) {
                t = localtime(&now);
                if ( t )
...
static void do_log_v(libiodef_log_t level, const char *file,
                     const char *function, int line, const char *fmt, va_list ap)
{
        ...
        if ( level <= LIBIODEF_LOG_ERR || level >= LIBIODEF_LOG_DEBUG ) {
        ...
        if ( need_to_log(level, log_level) || need_to_log(level, debug_level) ) {
                ...
                if ( ret < 0 && global_log_cb == do_log_print ) {
                        libiodef_log_set_flags(LIBIODEF_LOG_FLAGS_SYSLOG);
...
/**
 * _libiodef_log:
 * @level: LIBIODEF_LOG_PRIORITY_INFO or LIBIODEF_LOG_PRIORITY_ERROR.
 ...
 * Use the #log macro defined in libiodef-log.h
 */
...
/**
 * libiodef_log_set_flags:
 * @flags:
 */
void libiodef_log_set_flags(libiodef_log_flags_t flags)
{
        if ( flags & LIBIODEF_LOG_FLAGS_QUIET )
                log_level = LIBIODEF_LOG_WARN;

        if ( flags & LIBIODEF_LOG_FLAGS_SYSLOG )
...
{
        debug_level = LIBIODEF_LOG_DEBUG + level;
}



/**
 * libiodef_log_set_prefix:
...
/**
 * libiodef_log_set_callback:
 * @log_cb: Callback function.
 *
 * @log_cb() will be called in place of the libiodef function usally
 * used for loging.
 */
...
int _libiodef_log_set_abort_level_from_string(const char *level)
{
        ...
        struct {
                const char *prefix;
                libiodef_log_t level;
        } tbl[] = {
                { "CRIT",  LIBIODEF_LOG_CRIT  },
                { "ERR",   LIBIODEF_LOG_ERR   },
                { "WARN",  LIBIODEF_LOG_WARN  },
                { "INFO",  LIBIODEF_LOG_INFO  },
                { "DEBUG", LIBIODEF_LOG_DEBUG }
        };

        libiodef_return_val_if_fail(level != NULL, libiodef_error(LIBIODEF_ERROR_ASSERTION));

        for ( i = 0; i < sizeof(tbl) / sizeof(*tbl); i++ ) {
                if ( strncasecmp(tbl[i].prefix, level, strlen(tbl[i].prefix)) == 0 ) {
                        _libiodef_log_set_abort_level(tbl[i].level);
                        return 0;
                }
        }
        ...
        if ( eptr != (level + strlen(level)) || lvalue == LONG_MIN || lvalue == LONG_MAX ) {
                libiodef_log(LIBIODEF_LOG_WARN, "Invalid abort level specified: '%s'.\n", level);
                return -1;
        }

        _libiodef_log_set_abort_level(lvalue);
...
void libiodef_log(libiodef_log_t level, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        _libiodef_log_v(level, "", "", 0, fmt, ap);
        va_end(ap);
}
...
void libiodef_log_debug(libiodef_log_t level, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        _libiodef_log_v(LIBIODEF_LOG_DEBUG + level, "", "", 0, fmt, ap);
        va_end(ap);
}
