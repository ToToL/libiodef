...
#include "iodef.h"
#include "iodef-document-helpers.h"


/**
 * iodef_document_set_value:
 * @message: Pointer to an #iodef_document_t object.
...
int iodef_document_set_value(iodef_document_t *message, const char *path, iodef_value_t *value)
{
        ...
        ret = iodef_path_new_fast(&ip, path);
        if ( ret < 0 )
                return ret;

        ret = iodef_path_set(ip, message, value);
        iodef_path_destroy(ip);
...
int iodef_document_get_value(iodef_document_t *message, const char *path, iodef_value_t **value)
{
        ...
        ret = iodef_path_get(ip, message, value);
        iodef_path_destroy(ip);
 ...
 *
 * Example:
 * iodef_document_set_string(message, "alert.classification.text", "MyText");
 * iodef_document_set_string(message, "alert.source(0).service.port", "1024");
 *
 * Returns: 0 on success, or a negative value if an error occured.
 */
int iodef_document_set_string(iodef_document_t *message, const char *path, const char *value)
{
        ...
        ret = iodef_document_set_value(message, path, iv);
...
int iodef_document_get_string(iodef_document_t *message, const char *path, char **result)
{
        ...
        if ( iodef_value_get_type(iv) != IODEF_VALUE_TYPE_STRING ) {
                ret = _iodef_value_cast(iv, IODEF_VALUE_TYPE_STRING, 0);
                if ( ret < 0 )
                        goto err;
        }

        if ( ! (str = iodef_value_get_string(iv)) ) {
                ret = -1;
                goto err;
        }
 ...
 *
 * Example:
 * iodef_document_set_number(message, "alert.assessment.confidence.confidence", 0.123456);
 * iodef_document_set_number(message, "alert.source(0).service.port", 1024);
 *
 * Returns: 0 on success, or a negative value if an error occured.
 */
int iodef_document_set_number(iodef_document_t *message, const char *path, double number)
{
        ...
        ret = iodef_value_new_double(&iv, number);
        if ( ret < 0 )
                return ret;
...
int iodef_document_get_number(iodef_document_t *message, const char *path, double *result)
{
        ...
        if ( iodef_value_get_type(iv) != IODEF_VALUE_TYPE_DOUBLE ) {
                ret = _iodef_value_cast(iv, IODEF_VALUE_TYPE_DOUBLE, 0);
                if ( ret < 0 )
                        goto err;
        }

        ret = 1;
        *result = iodef_value_get_double(iv);
...
int iodef_document_set_data(iodef_document_t *message, const char *path, const unsigned char *data, size_t size)
{
        ...
        ret = iodef_data_new_byte_string_dup(&id, data, size);
        if ( ret < 0 )
                return ret;

        ret = iodef_value_new_data(&iv, id);
        if ( ret < 0 ) {
                iodef_data_destroy(id);
                return ret;
        }
...
int iodef_document_get_data(iodef_document_t *message, const char *path, unsigned char **data, size_t *size)
{
        ...
        if ( iodef_value_get_type(iv) != IODEF_VALUE_TYPE_DATA || ! (dp = iodef_value_get_data(iv)) ) {
                ret = -1;
                goto err;
        }

        *size = iodef_data_get_len(dp);
        *data = malloc(*size);
        if ( ! *data ) {
                ret = -1;
                goto err;
        ...
        memcpy(*data, iodef_data_get_data(dp), *size);
