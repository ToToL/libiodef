...
package GenerateIODEFTreeWrapC;
...
sub     struct_desc
{
    ...
    $self->output("\n\nstruct iodef_$struct->{short_typename} \{
");
...
sub     struct_constructor
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_new:
 ...
 */
int iodef_$struct->{short_typename}_new($struct->{typename} **ret)
\{
...
");
        ...
        $self->output("
                if ( retval < 0 ) {
                        iodef_$struct->{short_typename}_destroy(*ret);
                        *ret = NULL;
                        return retval;
                }
        }
...
sub     struct_get_child
{
    ...
    $self->output("
int _iodef_$struct->{short_typename}_get_child(void *p, iodef_class_child_id_t child, void **childptr)
\{
...
");
            ...
            $self->output("
                       return iodef_value_new_enum_from_numeric((iodef_value_t **) childptr,
                                                                IODEF_CLASS_ID_" . uc($field->{short_typename}) . ", ptr->$field->{short_name});
");
        } elsif ( $field->{metatype} & (&METATYPE_OPTIONAL_INT) ) {
           if ( $field->{metatype} & &METATYPE_OPTIONAL_INT ) {
            $self->output("
                case $n:
                       return (ptr->$field->{name}_is_set) ? iodef_value_new_$field->{value_type}((iodef_value_t **) childptr, ptr->$field->{name}) : 0;
");
        }
            ...
            if ( $field->{typename} eq "prelude_string_t" ) {
                ...
                $self->output("
                       return get_value_from_string((iodef_value_t **) childptr, $refer ptr->$field->{name}, $owned);");
            }
            elsif ( $field->{typename} eq "iodef_data_t" ) {
                my $refer = $field->{ptr} ? "" : "&";
                my $owned = $field->{ptr} ? "TRUE" : "FALSE";
                $self->output("
                       return get_value_from_data((iodef_value_t **) childptr, $refer ptr->$field->{name}, $owned);");
            } elsif ( $field->{typename} eq "iodef_time_t" ) {
                my $refer = $field->{ptr} ? "" : "&";
                my $owned = $field->{ptr} ? "TRUE" : "FALSE";
                $self->output("
                       return get_value_from_time((iodef_value_t **) childptr, $refer ptr->$field->{name}, $owned);");
            }

            else {
                $self->output("
                       return iodef_value_new_$field->{value_type}((iodef_value_t **) childptr, ptr->$field->{name});");
            }
...
sub     struct_destroy_child
{
    ...
    $self->output("
int _iodef_$struct->{short_typename}_destroy_child(void *p, iodef_class_child_id_t child, int n)
\{
        $struct->{typename} *ptr = p;
...
");
            ...
            foreach my $member ( @{ $field->{member_list} } ) {
                ...
                case $n:
                        ...
                        iodef_$member->{short_typename}_destroy(ptr->$field->{name}.$member->{name});
...
sub     struct_new_child
{
    ...
    $self->output("
int _iodef_$struct->{short_typename}_new_child(void *p, iodef_class_child_id_t child, int n, void **ret)
\{
        $struct->{typename} *ptr = p;
...
");
            ...
            foreach my $member ( @{ $field->{member_list} } ) {
                $self->output("
                case $n:
                        return iodef_$struct->{short_typename}_new_$member->{name}(ptr, ($member->{typename} **) ret);
");
                $n++;
            }
            ...
            $self->output("
                case $n:
                        return iodef_$struct->{short_typename}_new_$field->{name}(ptr, ($field->{typename} **) ret);
");
        }
...
sub     struct_copy
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_copy:
 ...
 * Returns: 0 on success, a negative value if an error occured.
 */
int iodef_$struct->{short_typename}_copy(const $struct->{typename} *src, $struct->{typename} *dst)
\{
        int ret;
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( ! ($field->{metatype} & &METATYPE_PRIMITIVE) ) {
            $clone_func = "iodef_${clone_func}";
            $copy_func = "iodef_${copy_func}";
            $destroy_func = "iodef_${destroy_func}";
        }
...
");
            ...
            foreach my $member ( @{ $field->{member_list} } ) {
                ...
                $self->output("
                case $member->{value}:
                        iodef_$member->{short_typename}_destroy(dst->$field->{name}.$member->{name});
                        break;
");
            }
            $self->output("
...
");
            ...
            foreach my $member ( @{ $field->{member_list} } ) {
                ...
                $self->output("
                case $member->{value}:
                        ret = iodef_$member->{short_typename}_clone(src->$field->{name}.$member->{name}, &dst->$field->{name}.$member->{name});
                        break;
");
            }
            $self->output("
...
sub     struct_clone
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_clone:
 ...
 * Returns: 0 on success, a negative value if an error occured.
 */
int iodef_$struct->{short_typename}_clone($struct->{typename} *src, $struct->{typename} **dst)
\{
        int ret;
        ...
        return iodef_$struct->{short_typename}_copy(src, *dst);
\}
");
}
...
sub     struct_cmp
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_compare:
 ...
 */
int iodef_$struct->{short_typename}_compare(const $struct->{typename} *obj1, const $struct->{typename} *obj2)
\{
        int ret = 0;
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( ! ($field->{metatype} & &METATYPE_PRIMITIVE) ) {
            $compare_func = "iodef_${compare_func}";
        }
...
");
            ...
            foreach my $member ( @{ $field->{member_list} } ) {
                ...
                $self->output("
                case $member->{value}:
                        ret = iodef_$member->{short_typename}_compare(obj1->$field->{name}.$member->{name}, obj2->$field->{name}.$member->{name});
                        break;
");
            }
            $self->output("
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( ! ($field->{metatype} & &METATYPE_PRIMITIVE) ) {
            $destroy_func = "iodef_${destroy_func}";
            $destroy_internal_func = "iodef_$destroy_internal_func";
        }
...
sub     struct_destroy
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_destroy:
...
sub     struct_ref
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_ref:
...
sub     struct_field_normal
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_get_${name}:
 * \@ptr: pointer to a #$struct->{typename} object.
 *
...
");
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_set_$field->{name}:
 * \@ptr: pointer to a #$struct->{typename} object.
...
");
        ...
        if ( $field->{ptr} ) {
            ...
            $destroy_func = "iodef_${destroy_func}" if ( ! ($field->{metatype} & &METATYPE_PRIMITIVE) );
...
");
            ...
            $destroy_internal_func = "iodef_${destroy_internal_func}" if ( ! ($field->{metatype} & &METATYPE_PRIMITIVE) );
...
");
        ...
        if ( $struct->{typename} eq "iodef_additional_data_t" and $name eq "type" ) {
                $self->output("
...
");
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_new_${name}:
 * \@ptr: pointer to a #$struct->{typename} object.
 ...
 */
int iodef_$struct->{short_typename}_new_${name}($struct->{typename} *ptr, $field->{typename} **ret)
\{");
    my $need_check = 1;
...
");
    ...
    if ( $field->{typename} eq "iodef_additional_data_type_t" and $field->{name} eq "type" ) {
...
sub     struct_field_union
{
    ...
    $self->output("
/**
 * iodef_$struct->{short_typename}_get_$field->{var}:
 * \@ptr: pointer to a #$struct->{typename} object.
 *
...
");
    ...
    foreach my $member ( @{ $field->{member_list} } ) {
        $self->output("
/**
 * iodef_$struct->{short_typename}_get_$member->{name}:
 * \@ptr: pointer to a #$struct->{typename} object.
 *
...
"
        ...
        $self->output("
/**
 * iodef_$struct->{short_typename}_set_$member->{name}:
 * \@ptr: pointer to a #$struct->{typename} object.
...
");
        ...
        $self->output("
/**
 * iodef_$struct->{short_typename}_new_$member->{name}:
 * \@ptr: pointer to a #$struct->{typename} object.
 ...
 * Returns: 0 on success, or a negative value if an error occured.
 */
int iodef_$struct->{short_typename}_new_$member->{name}($struct->{typename} *ptr, $member->{typename} **ret)
\{
        int retval;
...
");
                ...
                case $other_member->{value}:
                        iodef_$other_member->{short_typename}_destroy(ptr->$field->{name}.$other_member->{name});
                        break;
");
            }
        }
        ...
        retval = iodef_$member->{short_typename}_new(ret);
        if ( retval < 0 )
                return retval;
...
sub     struct_field_list
{
    ...
    $new_field_function = "iodef_${new_field_function}" if ( ! ($field->{metatype} & &METATYPE_PRIMITIVE) );

    $self->output("
/**
 * iodef_$struct->{short_typename}_get_next_$field->{short_name}:
 * \@$struct->{short_typename}: pointer to a #$struct->{typename} object.
...
/**
 * iodef_$struct->{short_typename}_set_$field->{short_name}:
 * \@ptr: pointer to a #$struct->{typename} object.
...
/**
 * iodef_$struct->{short_typename}_new_$field->{short_name}:
 * \@ptr: pointer to a #$struct->{typename} object.
 * \@ret: pointer to an address where to store the created #$field->{typename} object.
 * \@pos: position in the list.
 *
 ...
 * Returns: 0 on success, or a negative value if an error occured.
 */
int iodef_$struct->{short_typename}_new_$field->{short_name}($struct->{typename} *ptr, $field->{typename} **ret, int pos)
\{
        int retval;
...
sub        enum
{
    ...
    $self->output("
/**
 * iodef_$enum->{short_typename}_to_numeric:
 * \@name: pointer to an IODEF string representation of a #$enum->{typename} value.
 *
 * Returns: the numeric equivalent of \@name, or -1 if \@name is not valid.
 */
$enum->{typename} iodef_$enum->{short_typename}_to_numeric(const char *name)
\{
        size_t i;
        const struct {
              $enum->{typename} val;
    ...
    foreach my $field ( @{ $enum->{field_list} } ) {
        ...
        if ( $field->{text} ) {
            ...
            my $fullname = 'IODEF_' . uc($enum->{short_typename});
...
");
        ...
        $self->output("
/**
 * iodef_$enum->{short_typename}_to_string:
 * \@val: an enumeration value for #$enum->{typename}.
 *
 * Return the IODEF string equivalent of \@val provided #$enum->{typename} value.
 *
 * Returns: a pointer to the string describing \@val, or NULL if \@val is invalid.
 */
const char *iodef_$enum->{short_typename}_to_string($enum->{typename} val)
\{
        const struct {
              $enum->{typename} val;
              const char *name;
