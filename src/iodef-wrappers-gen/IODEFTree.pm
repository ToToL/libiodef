...
sub	parse_struct
{
	...
	if ( $line =~ /^\s*HIDE\(/ ) {
	...
	} elsif ( $line =~ /^\s*IODEF_LINKED_OBJECT\s*\;\s*$/ ) {
	    $struct->{is_listed} = 1;
	    ...
	    push(@field_list, { metatype => &METATYPE_NORMAL | &METATYPE_STRUCT,
				...
				short_typename => get_iodef_name($typename),
				name => $name,
				short_name => $name,
				ptr => $ptr ? 1 : 0,
				dynamic_ident => 0 });
	    ...
	    $struct->{short_typename} = get_iodef_name($typename);
	    $struct->{id} = $id;
...
sub	parse_enum
{
	...
	if ( ($name, $empty, $text, $value) = $line =~ /^\s*([^\( ]+)(\(([^\)]+)*\))?\s*\=\s*(\d+)/ ) {
	    ...
	    $enum->{short_typename} = get_iodef_name($typename);
	    $enum->{id} = $id;
...
sub	parse_pre_declared
{
    ...
    $pre_declared->{short_typename} = get_iodef_name($typename);
