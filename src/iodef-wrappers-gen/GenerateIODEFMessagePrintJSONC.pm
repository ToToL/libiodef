...
package GenerateIODEFMessagePrintJSONC;
...
");
        ...
        $self->output("
                \{
                        const char *enumstr = iodef_$field->{short_typename}_to_string(${refer}i);
...
");
        ...
        \{
                ...
                if ( field ) \{
                        ...
                        ret = iodef_$field->{short_typename}_print_json(field, fd);
                        if ( ret < 0 )
                                return ret;
                \}
        \}
...
sub        struct_field_union
{
    ...
    foreach my $member ( @{ $field->{member_list} } ) {
        ...
        case $member->{value}:
                ...
                ret = iodef_$member->{name}_print_json(iodef_$struct->{short_typename}_get_$member->{name}(ptr), fd);
                if ( ret < 0 )
                        return ret;
                break;
");
...
");
        ...
        $self->output("
                        ret = iodef_$field->{short_typename}_print_json(elem, fd);
                        if ( ret < 0 )
                                return ret;
");
    }
...
/**
 * iodef_$struct->{short_typename}_json:
