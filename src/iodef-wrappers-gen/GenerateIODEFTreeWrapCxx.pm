...
package GenerateIODEFTreeWrapCxx;
...
/* Auto-generated by the GenerateIODEFTreeWrapCppBindings package */

#include \"config.h\"

#include \"iodef-tree-wrap.hxx\"
#include \"common.h\"
 ...
 */
IODEF$struct->{short_typename}::IODEF$struct->{short_typename}()
\{
        iodef_$struct->{short_typename}_new(&_priv);
\}
");
}
 ...
 * Returns: 0 on success, a negative value if an error occured.
 */
int IODEF$struct->{short_typename}::copy(IODEF$struct->{short_typename} *dst)
\{
        return iodef_$struct->{short_typename}_copy(_priv, dst->_priv);
\}
");
}
 ...
 * Returns: 0 on success, a negative value if an error occured.
 */
IODEF$struct->{short_typename} *IODEF$struct->{short_typename}::clone()
\{
        int ret;
        $struct->{typename} *dst;
        IODEF$struct->{short_typename} *ptr;
 ...
 */
int IODEF$struct->{short_typename}::compare(IODEF$struct->{short_typename} *obj)
\{
        return iodef_$struct->{short_typename}_compare(_priv, obj->_priv);
\}
");
}
...
IODEF$struct->{short_typename}::~IODEF$struct->{short_typename}()
\{
        iodef_$struct->{short_typename}_destroy(_priv);
\}
");
}
...
sub     struct_field_normal
{
    ...
    $self->output("
$type ${ptr} IODEF$struct->{short_typename}::get_${name}()
\{
        return $tocast(iodef_$struct->{short_typename}_get_${name}(_priv));
\}
");
...
void IODEF$struct->{short_typename}::unset_$field->{name}()
\{
        iodef_$struct->{short_typename}_unset_$field->{name}(_priv);
\}
");
...
void IODEF$struct->{short_typename}::set_$field->{name}($field->{typename} *$field_name)
\{
        iodef_$struct->{short_typename}_set_$field->{name}(this, $field_name);
\}
");
        } else {
 ...
 */
$field->{typename} IODEF$struct->{short_typename}::get_$field->{var}()
\{
        return iodef_$struct->{short_typename}_get_$field->{var}(_priv);
\}
");
 ...
 */
$member->{typename} *IODEF$struct->{short_typename}::get_$member->{name}()
\{
        return iodef_$struct->{short_typename}_get_$member->{name}(_priv);
\}
"
);
 ...
 */
void IODEF$struct->{short_typename}::set_$member->{name}($member->{typename} *$member->{name})
\{
        iodef_$struct->{short_typename}_set_$member->{name}(_priv, $member->{name});
\}
");
 ...
 */
IODEF$member->{short_typename} *IODEF$struct->{short_typename}::new_$member->{name}()
\{
        int ret;
        $member->{typename} *ptr;
        IODEF$member->{short_typename} *obj;
 ...
 */
$field->{typename} *IODEF$struct->{short_typename}::get_next_$field->{short_name}($field->{typename} *$field->{short_typename}_cur)
\{
        return iodef_$struct->{short_typename}_get_next_$field->{short_name}(_priv, $field->{short_typename}_cur);
\}
 ...
 */
void IODEF$struct->{short_typename}::set_$field->{short_name}($field->{typename} *object, int pos)
\{
        iodef_$struct->{short_typename}_set_$field->{short_name}(_priv, object, pos);
\}
 ...
 * Returns: 0 on success, or a negative value if an error occured.
 */
int IODEF$struct->{short_typename}::new_$field->{short_name}($field->{typename} **ret, int pos)
\{
        return iodef_$struct->{short_typename}_new_$field->{short_name}(_priv, ret, pos);
\}
 ...
 */
$enum->{typename} IODEF$enum->{short_typename}::$enum->{short_typename}_to_numeric(const char *name)
\{
        return iodef_$enum->{short_typename}_to_numeric(name);
\}
");
 ...
 */
const char *IODEF$enum->{short_typename}::to_string($enum->{typename} val)
\{
        return iodef_$enum->{short_typename}_to_string(val);
\}
");
}
 ...
 * The objects are only destroyed if their reference count reach zero.
 */
void IODEFMessage::~IODEFMessage()
\{
        iodef_document_destroy(_priv);
\}
");
