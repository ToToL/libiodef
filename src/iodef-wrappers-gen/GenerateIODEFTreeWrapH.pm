...
package GenerateIODEFTreeWrapH;
...
");
    ...
    $self->output("\n");
    $self->output("#endif /* _LIBPRELUDE_IODEF_TREE_WRAP */\n");
}
...
sub     struct_definition
{
    ...
    $self->output(" */\n\n");
    $self->output("
typedef struct iodef_$struct->{short_typename} $struct->{typename};
...
sub     struct_constructor
{
    ...
    $self->output("int iodef_$struct->{short_typename}_new($struct->{typename} **ret);\n");
    $self->output("int iodef_$struct->{short_typename}_copy(const $struct->{typename} *src, $struct->{typename} *dst);\n");
    $self->output("int iodef_$struct->{short_typename}_clone($struct->{typename} *src, $struct->{typename} **dst);\n");
    $self->output("int iodef_$struct->{short_typename}_compare(const $struct->{typename} *obj1, const $struct->{typename} *obj2);\n");
}
...
sub     struct_ref
{
    ...
    $self->output("$struct->{typename} *iodef_$struct->{short_typename}_ref($struct->{typename} *$struct->{short_typename});\n");
...
sub     struct_destroy_child
{
    ...
    $self->output("int _iodef_$struct->{short_typename}_destroy_child(void *p, iodef_class_child_id_t child, int n);\n");
}
...
sub     struct_get_child
{
    ...
    $self->output("int _iodef_$struct->{short_typename}_get_child(void *p, iodef_class_child_id_t child, void **childptr);\n");
}
...
sub     struct_new_child
{
    ...
    $self->output("int _iodef_$struct->{short_typename}_new_child(void *p, iodef_class_child_id_t child, int n, void **ret);\n");
}
...
sub     struct_destroy
{
    ...
    $self->output("void iodef_$struct->{short_typename}_destroy($struct->{typename} *ptr);\n");
}
...
sub     struct_field_normal
{
    ...
    if ( $field->{metatype} & &METATYPE_OPTIONAL_INT ) {
        $self->output("void iodef_$struct->{short_typename}_unset_${name}($struct->{typename} *ptr);\n");
    }

    $self->output("$field->{typename} ${refer}iodef_$struct->{short_typename}_get_${name}($struct->{typename} *ptr);\n");

    my $field_name = ($name eq "class") ? "class_str" : $name;

    $self->output("void iodef_$struct->{short_typename}_set_${name}($struct->{typename} *ptr, $field->{typename} ${ptr}${field_name});\n");

    $self->output("int iodef_$struct->{short_typename}_new_${name}($struct->{typename} *ptr, $field->{typename} **ret);\n");
...
sub     struct_field_union
{
    ...
    $self->output("$field->{typename} iodef_$struct->{short_typename}_get_$field->{var}($struct->{typename} *ptr);\n");
...
sub     obj
{
    ...
    push(@{ $self->{type_list} }, [ "IODEF_CLASS_ID_" . uc("$obj->{short_typename}") => $obj->{id} ])
